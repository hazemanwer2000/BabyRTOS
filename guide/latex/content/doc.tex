...
\xchapter{System Calls}{chapter:system-calls}
	...
	\xsection{Unprivileged mode and \texttt{SVC} calls}{section:unprivileged-svc}
		\xpara{
			The Cortex-M4 has privileged and unprivileged operations modes in thread mode. Usually, for security purposes, tasks run in unprivileged mode. For example, a task in unprivileged mode may not switch to using \texttt{MSP}. Additionally, in unprivileged mode, the \texttt{PendSV} exception may not be triggered. 
		}
		\xpara{
